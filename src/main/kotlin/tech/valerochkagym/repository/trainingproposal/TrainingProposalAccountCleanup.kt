 ...
 * through their existing owner FK. Author-owned proposals for other recipients are never selected.
 */
@Repository
class TrainingProposalAccountCleanup(
  ...
  fun preflightRecipient(userId: UUID) {
    catalog.readLock()
    heads.writeLockOrNull(userId)
  }

  /** Requires [preflightRecipient] in the containing transaction. */
