...
@Component
class HealthRawBodyReader(private val json: ObjectMapper) {
  fun raw(request: HttpServletRequest, maxBody: Int = 5242880): ByteArray {
    ...
    val charset =
      request.contentType
        ...
        ?.substringAfter('=')
        ?.trim('"')
    if (charset != null && !charset.equals("UTF-8", true)) bad("Требуется UTF-8")
    val bytes =
  ...
  fun tree(raw: ByteArray): JsonNode {
