...
import tech.valerochkagym.web.bad
...
class RecordValidator {
  ...
  private fun equipment(n: JsonNode) {
    ...
    if (
      entries.any { !it.isString || it.asString() !in EquipmentDefinitions.provides } ||
  ...
  fun references(records: Map<RecordKey, Record>) {
      ...
      .forEach { r ->
        ...
        when (r.kind) {
          ...
          "workout" -> {
            ...
            if (r.kind == "routine" || n["finishedAt"] == null || n["finishedAt"].isNull) {
                  ...
                  val available =
                    if (!gym["inventoryConfigured"].asBoolean()) {
                      ...
                      val coverage =
                        gym["equipmentIds"]
                          .toList()
                          .flatMap { EquipmentDefinitions.provides[it.asString()].orEmpty() }
                          .toSet()
